import { Box, Button, Card, CardContent, Chip, CircularProgress, IconButton, Paper, Skeleton, Stack, Table, TableBody, TableCell, TableContainer, TableFooter, TableHead, TablePagination, TableRow, Tooltip, Typography, useTheme } from "@mui/material"
import Header from "../../layout/Header"
import { tokens } from "../../theme";
import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import ApiService from "../../services/ApiService";
import type { WarehouseFormData, WarehousesData, WarehouseWithTotalChangedQtyData } from "../../types/warehouse";
...
const WarehousePage = () => {
    ...
    const { isLoading, error, data } = useQuery<{
        warehouseDatas: WarehousesData[];
        warehouseWithTotalDatas: WarehouseWithTotalChangedQtyData[];
    }>({
        queryKey: ['WarehousesData'],
        queryFn: async () => {
            const resWarehouses = await ApiService.getAllWarehouses();
            const resWarehousesWithTotal = await ApiService.getWarehouseWithTotalChangedQty();
            return {
                warehouseDatas: resWarehouses.data,
                warehouseWithTotalDatas: resWarehousesWithTotal.data
            }
        }
    });
    ...
    const createMutation = useMutation({
        mutationFn: async (data: WarehouseFormData) => {
            const resAddedWarehouse = await ApiService.createWarehouse(data);
            setAddedWarehouseIndex(resAddedWarehouse.data.id);

            return resAddedWarehouse;
        },
        onSuccess: () => {
            showSnackbar("倉庫を作成しました", "success");
            queryClient.invalidateQueries({ queryKey: ['WarehousesData'] });

        },
        onError: (error: AxiosError<{ message: string }>) => {
            showSnackbar(error.response?.data?.message || "作成に失敗しました", "error");
        }
    });

    const deleteMutation = useMutation({
        mutationFn: async (id: number) => {
            return ApiService.deleteWarehouse(id);
        },
        onSuccess: () => {
            showSnackbar("倉庫を削除しました", "success");
            queryClient.invalidateQueries({ queryKey: ['WarehousesData'] });
    ...
    const updateMutation = useMutation({
        mutationFn: async ({ id, data }: { id: number; data: WarehouseFormData }) => {
            return ApiService.updateWarehouse(id, data);
        },
        onSuccess: () => {
            showSnackbar("倉庫を編集しました", "success");
            queryClient.invalidateQueries({ queryKey: ['WarehousesData'] });
        },
        onError: (error: AxiosError<{ message: string }>) => {
    ...
    useEffect(() => {
        console.log(data?.warehouseDatas)
        if (data?.warehouseDatas && addedWarehouseIndex) {
            const newIndex = data.warehouseDatas.findIndex(w => w.id === addedWarehouseIndex);
            setCurrentWarehouseIndex(newIndex);
        }
    }, [addedWarehouseIndex, data?.warehouseDatas])
    useEffect(() => {
        if (data?.warehouseDatas && data.warehouseDatas.length > 0) {
            const firstWarehouse = data.warehouseDatas[currentWarehouseIndex];
            if (firstWarehouse) {
                setSelectedWarehouse(firstWarehouse);
                const firstWarehouseWithTotal = data.warehouseWithTotalDatas.find(
                    wh => wh.id === firstWarehouse.id
                );
                setSelectedWarehouseWithTotal(firstWarehouseWithTotal);
            }
        }
    }, [data, currentWarehouseIndex]);

    const handleNextWarehouse = () => {
        if (data?.warehouseDatas && data.warehouseDatas.length > 0) {
            const nextIndex = (currentWarehouseIndex + 1) % data.warehouseDatas.length;
            setCurrentWarehouseIndex(nextIndex);
            const nextWarehouse = data.warehouseDatas[nextIndex];
            setSelectedWarehouse(nextWarehouse);

            const nextWarehouseWithTotal = data.warehouseWithTotalDatas.find(
    ...
    const handleBackWarehouse = () => {
        if (data?.warehouseDatas && data.warehouseDatas.length > 0) {
            const prevIndex = (currentWarehouseIndex - 1 + data.warehouseDatas.length) % data.warehouseDatas.length;
            setCurrentWarehouseIndex(prevIndex);
            const prevWarehouse = data.warehouseDatas[prevIndex];
            setSelectedWarehouse(prevWarehouse);

            const prevWarehouseWithTotal = data.warehouseWithTotalDatas.find(
    ...
    return (
        <Box m={3}>
            {isLoading ? (
                ...
                <Header
                    title={`倉庫(${data?.warehouseDatas?.length ?? 0})`}
                    subtitle="倉庫の詳細情報を表示"
                />
            )}
            <Box
                ...
                {isLoading ? (
                    ...
                    >
                        ...
                        >
                            ...
                            {data?.warehouseDatas && data.warehouseDatas.map((wh) => (
                                <Button
                                    key={wh.id}
                                    color="info"
                                    ...
                                    onClick={() => {
                                        ...
                                        const warehouseWithTotal = data.warehouseWithTotalDatas.find(
                                            whWithTotal => whWithTotal.id === wh.id
                                        );
                                        setSelectedWarehouseWithTotal(warehouseWithTotal);

                                        const newIndex = data.warehouseDatas.findIndex(w => w.id === wh.id);
                                        setCurrentWarehouseIndex(newIndex);
                                        setPage(0);
                                    }}
                                >
                        ...
                        >
                            <Tooltip title="戻">
                                <IconButton onClick={handleBackWarehouse}>
                                    <ArrowBackIosIcon />
                                </IconButton>
                            </Tooltip>
                            <Tooltip title="次">
                                <IconButton onClick={handleNextWarehouse}>
                                    <ArrowForwardIosIcon />
                                </IconButton>
                            </Tooltip>
                            <Tooltip title="登録">
